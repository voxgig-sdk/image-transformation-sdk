// Command ImageTransformation-cli is a CLI + REPL for the ImageTransformation SDK,
...
const prompt = "image-transformation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "image_transformation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IMAGE_TRANSFORMATION_APIKEY (api key), IMAGE_TRANSFORMATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IMAGE_TRANSFORMATION_APIKEY carries the API key and
	// IMAGE_TRANSFORMATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IMAGE_TRANSFORMATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IMAGE_TRANSFORMATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewImageTransformationSDK(opts)
