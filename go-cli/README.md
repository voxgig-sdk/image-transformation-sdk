...
boru-driven command-line client **and** interactive REPL for the ImageTransformation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/image-transformation-cli)
make build

# 2. See usage (words, entities, env vars)
./image-transformation-cli --help

# 3. Provide credentials once, via the environment
export IMAGE_TRANSFORMATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./image-transformation-cli load 1 image_transformation            # {id:1} shorthand
./image-transformation-cli load '{id:1}' image_transformation       # explicit match map

# 5. Override the API base URL for a single call
IMAGE_TRANSFORMATION_BASE=https://api.example.com ./image-transformation-cli load 1 image_transformation

# 6. No arguments -> interactive REPL
./image-transformation-cli
image-transformation> load 1 image_transformation
image-transformation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/image-transformation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/image-transformation-cli load 1 image_transformation
   ```
...
```sh
./image-transformation-cli load 1 image_transformation          # scalar shorthand for {id:1}
./image-transformation-cli load '{id:1}' image_transformation     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `image-transformation>`). Each line is
evaluated as its own boru expression:

```text
$ ./image-transformation-cli
image-transformation> load 1 image_transformation
image-transformation> /help
image-transformation> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `image_transformation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/image-transformation-cli`. |
...
(`./image-transformation-cli <expr>`) and interactively (the REPL), and expressions compose the
