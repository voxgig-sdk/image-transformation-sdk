...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IMAGETRANSFORMATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("IMAGETRANSFORMATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IMAGETRANSFORMATION_TEST_EXPLAIN"); explain != "" {
