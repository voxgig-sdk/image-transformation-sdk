...
import (
	...
	omni "github.com/voxgig-sdk/image-transformation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ImageTransformationError); is {
				out := map[string]any{
					"name":    "ImageTransformationError",
...
func omniCtx(arg any, client *sdk.ImageTransformationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ImageTransformationSDK); is {
			client = live
