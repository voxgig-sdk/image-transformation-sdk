...
require("image-transformation_types")
...
local ImageTransformationSDK = {}
ImageTransformationSDK.__index = ImageTransformationSDK
...
ImageTransformationSDK._make_feature = _make_feature


function ImageTransformationSDK.new(options)
  local self = setmetatable({}, ImageTransformationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ImageTransformationSDK:options_map()
...
function ImageTransformationSDK:get_utility()
  return Utility.copy(self._utility)
end


function ImageTransformationSDK:get_root_ctx()
  return self._rootctx
end


function ImageTransformationSDK:prepare(fetchargs)
...
function ImageTransformationSDK:direct(fetchargs)
...
function ImageTransformationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ImageTransformationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ImageTransformationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ImageTransformationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ImageTransformationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ImageTransformationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ImageTransformationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ImageTransformationSDK.new(sdkopts)
  sdk.mode = "test"
