...
local ImageTransformationEntity = {}
ImageTransformationEntity.__index = ImageTransformationEntity


function ImageTransformationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ImageTransformationEntity)
  self._name = "image_transformation"
  self._client = client
...
function ImageTransformationEntity:get_name()
  return self._name
end


function ImageTransformationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ImageTransformationEntity.new(self._client, opts)
end
...
function ImageTransformationEntity:mark_deleted()
  self._deleted = true
end


function ImageTransformationEntity:deleted()
  return true == self._deleted
end


function ImageTransformationEntity:data_set(args)
  if args ~= nil then
...
function ImageTransformationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ImageTransformationEntity:match_set(args)
  if args ~= nil then
...
function ImageTransformationEntity:match_get()
...
function ImageTransformationEntity:stream(action, args, callopts)
...
function ImageTransformationEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ImageTransformationEntity
