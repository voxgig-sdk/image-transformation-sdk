# ImageTransformation Lua SDK



The Lua SDK for the ImageTransformation API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:ImageTransformation()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/image-transformation-sdk/releases)),
...
```
lua/
├── image-transformation_sdk.lua    -- Main SDK module
...
The main module (`image-transformation_sdk`) exports the SDK constructor
