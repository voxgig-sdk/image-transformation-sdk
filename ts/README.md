# ImageTransformation TypeScript SDK



The TypeScript SDK for the ImageTransformation API — a type-safe, entity-oriented client with full async/await support.
...
```ts
try {
  const imagetransformation = await client.ImageTransformation().load({ id: 'example_id' })
...
| `tester(testopts?, sdkopts?)` | `ImageTransformationSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `ImageTransformationSDK.test(testopts?, sdkopts?)` | `ImageTransformationSDK` | Create a test-mode client. |
...
| `client` | `client(): ImageTransformationSDK` | Return the parent SDK client. |
...
├── src/
│   ├── ImageTransformationSDK.ts        # Main SDK class
