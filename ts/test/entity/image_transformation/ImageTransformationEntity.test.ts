...
describe('ImageTransformationEntity', async () => {
  ...
  // `test.live.delayMs`; only sleeps when IMAGETRANSFORMATION_TEST_LIVE=TRUE.
  afterEach(liveDelay('IMAGETRANSFORMATION_TEST_LIVE'))
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'image_transformation.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['IMAGE_TRANSFORMATION_TEST_IMAGE_TRANSFORMATION_ENTID']
