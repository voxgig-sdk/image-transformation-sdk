...
describe('ImageTransformationDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new ImageTransformationSDK({
  ...
  test('direct-load-image_transformation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-image_transformation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['IMAGE_TRANSFORMATION_TEST_IMAGE_TRANSFORMATION_ENTID']
