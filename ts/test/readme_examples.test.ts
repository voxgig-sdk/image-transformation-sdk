...
//     (\`new ImageTransformationSDK(...)\` / \`ImageTransformationSDK.test(...)\` become
//     \`ImageTransformationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ImageTransformationSDK'
...
const TEST_SEED = {"entity":{"image_transformation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ImageTransformationSDK(...)\` and any
// \`ImageTransformationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ImageTransformationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ImageTransformationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
