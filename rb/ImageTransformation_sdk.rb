...
require_relative 'ImageTransformation_types'
...
class ImageTransformationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ImageTransformationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ImageTransformationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ImageTransformationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ImageTransformationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ImageTransformationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ImageTransformationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ImageTransformationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ImageTransformationError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ImageTransformationError => err
    ...
    if fetched.is_a?(Hash)
      status = ImageTransformationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ImageTransformationError.new(
        "graphql_error", "ImageTransformationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ImageTransformationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
