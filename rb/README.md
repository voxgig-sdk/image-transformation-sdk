# ImageTransformation Ruby SDK



The Ruby SDK for the ImageTransformation API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.ImageTransformation` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = ImageTransformationSDK.test(testopts, sdkopts)
```
...
raise a `ImageTransformationError` (a `StandardError` subclass), so wrap
...
```
rb/
├── ImageTransformation_sdk.rb       -- Main SDK module
...
The main module (`ImageTransformation_sdk`) exports the SDK class
