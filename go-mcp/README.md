...
[MCP](https://modelcontextprotocol.io) server exposing the ImageTransformation SDK as
two agent tools — `image-transformation_list` and `image-transformation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/image-transformation-mcp)
make build

# 2. Provide credentials via the environment
export IMAGE_TRANSFORMATION_APIKEY=sk_live_xxx
...
```jsonc
// image-transformation_list: first page of records
{ "entity": "image_transformation" }
{ "entity": "image_transformation", "query": { } }

// image-transformation_load: one record by id
{ "entity": "image_transformation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/image-transformation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user image-transformation \
     -- "$PWD"/dist/*/image-transformation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `image-transformation_list` and `image-transformation_load` tools now appear
   in new sessions. Ask the agent to *"list image_transformation using image-transformation"*
   and it calls `image-transformation_list` with `{"entity":"image_transformation"}`.
...
```sh
export IMAGE_TRANSFORMATION_APIKEY=sk_live_xxx            # API key
export IMAGE_TRANSFORMATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `image-transformation_list` tool
...
### Call the `image-transformation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `image-transformation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `image-transformation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IMAGE_TRANSFORMATION_APIKEY` | API key sent with every request. |
| `IMAGE_TRANSFORMATION_BASE` | Optional override of the API base URL. |
