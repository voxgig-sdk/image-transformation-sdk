# ImageTransformation SDK test runner

from __future__ import annotations
import os
import json

from imagetransformation_sdk.utility.voxgig_struct import voxgig_struct as vs


class ImageTransformationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ImageTransformationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ImageTransformationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ImageTransformationTestRunner.getenv("IMAGE_TRANSFORMATION_TEST_LIVE")
        override = ImageTransformationTestRunner.getenv("IMAGE_TRANSFORMATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ImageTransformationTestRunner.getenv(key)
        ...
        explain = ImageTransformationTestRunner.getenv("IMAGE_TRANSFORMATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ImageTransformationTestRunner._test_control is not None:
            return ImageTransformationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ImageTransformationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ImageTransformationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ImageTransformationTestRunner.load_test_control()
...
def load_env_local():
    ImageTransformationTestRunner.load_env_local()


def env_override(m):
    return ImageTransformationTestRunner.env_override(m)


def entity_data(v):
    return ImageTransformationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ImageTransformationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ImageTransformationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ImageTransformationTestRunner.load_test_control()


def live_client_options():
    return ImageTransformationTestRunner.live_client_options()


def live_delay_ms():
    return ImageTransformationTestRunner.live_delay_ms()
