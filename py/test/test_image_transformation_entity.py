    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IMAGE_TRANSFORMATION_TEST_IMAGE_TRANSFORMATION_ENTID JSON to run live")
        client = setup["client"]
        ...
        image_transformation_ref01_data_dt0_loaded = image_transformation_ref01_ent.load(image_transformation_ref01_match_dt0, None)
...
def _image_transformation_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IMAGE_TRANSFORMATION_TEST_IMAGE_TRANSFORMATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("IMAGE_TRANSFORMATION_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("IMAGE_TRANSFORMATION_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("IMAGE_TRANSFORMATION_TEST_EXPLAIN") == "TRUE",
        "live": _live,
