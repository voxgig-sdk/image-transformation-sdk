...
class ImageTransformationSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ImageTransformationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ImageTransformationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ImageTransformationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ImageTransformationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ImageTransformationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ImageTransformationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ImageTransformationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ImageTransformationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ImageTransformationError($op . "_allow",
                "ImageTransformationSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ImageTransformationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ImageTransformationError("graphql_error",
                "ImageTransformationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function ImageTransformation($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_image_transformation === null) {
                $this->_image_transformation = new ImageTransformationEntity($this, null);
            }
            return $this->_image_transformation;
        }
        return new ImageTransformationEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new ImageTransformationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
