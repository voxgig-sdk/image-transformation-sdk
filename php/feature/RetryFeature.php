...
// ImageTransformation SDK retry feature
...
class ImageTransformationRetryFeature extends ImageTransformationBaseFeature
{
    ...
    public function init(ImageTransformationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ImageTransformationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ImageTransformationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ImageTransformationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
