...
// ImageTransformation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ImageTransformationOmni::makeStructRunner(
    ...
    {
        return ImageTransformationOmni::objmodel(ImageTransformationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return ImageTransformationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return ImageTransformationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ImageTransformationOmni::structfix($now, Struct::undef()))
                        !== json_encode(ImageTransformationOmni::structfix($val, Struct::undef()))) {
