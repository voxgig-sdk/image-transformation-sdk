...
// ImageTransformation SDK feature test
//
...
class FtCtrl extends ImageTransformationControl
{
...
class FtClient
{
    ...
    private ImageTransformationUtility $utility;

    public function __construct(ImageTransformationUtility $utility, array $options)
    {
    ...
    public function get_utility(): ImageTransformationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ImageTransformationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ImageTransformationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ImageTransformationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ImageTransformationConfig::make_feature($name);
    ...
    private static function buildUrl(ImageTransformationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ImageTransformationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ImageTransformationResponse($fetched);
        }
    ...
    private function fail(ImageTransformationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ImageTransformationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
