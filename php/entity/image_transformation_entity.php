...
// ImageTransformation SDK ImageTransformation entity
...
class ImageTransformationEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "image_transformation";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new ImageTransformationEntity($this->_client, $opts);
    }

    /**
     * @param ImageTransformation|array $args ImageTransformation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ImageTransformationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ImageTransformation|array The current ImageTransformation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ImageTransformation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ImageTransformationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ImageTransformation fields).
     */
    ...
    /**
     * Load a single ImageTransformation.
     *
     * @param ImageTransformationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ImageTransformationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ImageTransformation|array The loaded ImageTransformation as an assoc-array at the
     *   SDK boundary; throws ImageTransformationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ImageTransformationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
