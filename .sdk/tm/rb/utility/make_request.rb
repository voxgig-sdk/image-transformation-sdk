...
module ImageTransformationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ImageTransformationResponse.new({})
    ...
    if fetch_err
      ...
      response = ImageTransformationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ImageTransformationResponse.new(fetched)
    else
