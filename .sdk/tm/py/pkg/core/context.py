...
from projectname_sdk.core.control import ImageTransformationControl
...
class ImageTransformationContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ImageTransformationControl()
        ...
        if isinstance(sp, ImageTransformationSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ImageTransformationResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ImageTransformationResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ImageTransformationOperation({})
        ...
        op = ImageTransformationOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return ImageTransformationError(code, msg, self)
