...
// ImageTransformation SDK pipeline test
//
...
{
    private static function utility(): ImageTransformationUtility
    {
        return new ImageTransformationUtility();
    }
    ...
    private static function ctx(array $over = []): ImageTransformationContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ImageTransformationContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ImageTransformationOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ImageTransformationError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ImageTransformationOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ImageTransformationMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ImageTransformationOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ImageTransformationMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ImageTransformationOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ImageTransformationMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ImageTransformationSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ImageTransformationMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ImageTransformationResponse([]),
            'result' => new ImageTransformationResult([]),
        ]);
        [, $err] = ImageTransformationMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ImageTransformationSpec([]),
            'response' => null,
            'result' => new ImageTransformationResult([]),
        ]);
        [, $err] = ImageTransformationMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ImageTransformationSpec(['step' => 's']),
            'response' => new ImageTransformationResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ImageTransformationResult([]),
        ]);
        [, $err] = ImageTransformationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ImageTransformationSpec(['step' => 's']),
            'response' => new ImageTransformationResponse(self::resp(200, ['v' => 1])),
            'result' => new ImageTransformationResult([]),
        ]);
        [, $err] = ImageTransformationMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ImageTransformationSpec(['step' => 's']),
            'response' => new ImageTransformationResponse(self::resp(200, ['v' => 2])),
            'result' => new ImageTransformationResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ImageTransformationMakeResponse::call($ctx);
    ...
    {
        $preset = new ImageTransformationResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ImageTransformationSpec([]),
            'response' => new ImageTransformationResponse([]),
            'result' => new ImageTransformationResult([]),
        ...
        [$got, $err] = ImageTransformationMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ImageTransformationResult([])]);
        [, $err] = ImageTransformationMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ImageTransformationSpec([]), 'result' => null]);
        [, $err] = ImageTransformationMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ImageTransformationSpec(['step' => 's']),
            'result' => new ImageTransformationResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ImageTransformationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ImageTransformationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ImageTransformationSpec(['step' => 's']),
            'result' => new ImageTransformationResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ImageTransformationOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ImageTransformationMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ImageTransformationResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ImageTransformationSpec([]),
            'result' => new ImageTransformationResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ImageTransformationMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ImageTransformationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ImageTransformationError('boom', 'boom');
        $utility->fetcher = function (ImageTransformationContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ImageTransformationSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ImageTransformationMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ImageTransformationContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ImageTransformationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ImageTransformationResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ImageTransformationMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ImageTransformationContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ImageTransformationSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ImageTransformationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ImageTransformationResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ImageTransformationSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ImageTransformationMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ImageTransformationMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ImageTransformationMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ImageTransformationResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ImageTransformationDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ImageTransformationResult(['ok' => false])]);
        $this->expectException(ImageTransformationError::class);
        ImageTransformationDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ImageTransformationResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ImageTransformationMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ImageTransformationMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ImageTransformationBaseFeature();
        $b = new ImageTransformationBaseFeature();
        ImageTransformationFeatureAdd::call($ctx, $a);
        ImageTransformationFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ImageTransformationBaseFeature
    {
        $f = new ImageTransformationBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ImageTransformationFeatureAdd::call($ctx, self::named_feature('a'));
        ImageTransformationFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ImageTransformationFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ImageTransformationFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ImageTransformationFeatureAdd::call($ctx, $replace);
        ...
        ImageTransformationFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ImageTransformationContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ImageTransformationSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ImageTransformationPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ImageTransformationPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ImageTransformationResponse(['headers' => null]),
            'result' => new ImageTransformationResult([]),
        ]);
        ImageTransformationResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ImageTransformationResultBody::call($ctx);
